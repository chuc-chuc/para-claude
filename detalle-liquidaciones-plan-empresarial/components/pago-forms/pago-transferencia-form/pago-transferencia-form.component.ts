// detalle-liquidaciones-plan-empresarial/components/pago-forms/pago-transferencia-form/pago-transferencia-form.component.ts
...
export class PagoTransferenciaFormComponent implements OnInit {
   ...
   */
  obtenerNombreBanco(idBanco: number): string {
    const banco = this.bancos.find(b => b.id_banco === idBanco);
    return banco?.nombre || 'Banco desconocido';
  }

  /**
   * Obtiene el nombre del tipo de cuenta por ID
   */
  obtenerTipoCuenta(idTipo: number): string {
    const tipo = this.tiposCuenta.find(t => t.id_tipo_cuenta === idTipo);
