...
final class tesoreriaApiClass extends ConexionBD
{
    ...
    private array $metodosPost = [
        ...
        'editarComprobanteTransferencia',
    ];
...
HTML;
     ...
     * POST: tesoreria/editarComprobanteTransferencia
     */
    public function editarComprobanteTransferencia($datos)
    {
        try {
            if (empty($datos->solicitud_id) || empty($datos->numero_registro_transferencia) ||
                empty($datos->fecha_transferencia)) {
            ...
            if (!in_array($solicitud['estado'], ['aprobado', 'completado'])) {
                return $this->res->fail('Solo se puede editar comprobante de solicitudes aprobadas o completadas');
            }
            ...
            $sql = "UPDATE compras.tesoreria_solicitudes_transferencia 
        SET numero_registro_transferencia = ?, 
            fecha_transferencia = ?,
            referencia_bancaria = ?, 
            observaciones_transferencia = CONCAT(COALESCE(observaciones_transferencia, ''), '\n\nEDITADO: ', ?),
            actualizado_por = ?
        WHERE id = ?";

            $stmt = $this->connect->prepare($sql);
            $stmt->execute([
                $datos->numero_registro_transferencia,
                $datos->fecha_transferencia,
                $datos->referencia_bancaria ?? null,
                $datos->observaciones ?? 'Sin observaciones adicionales',
                $this->idUsuario,
                $datos->solicitud_id
            ]);

            // Si está completado, actualizar también detalles de liquidación
            if ($solicitud['estado'] === 'completado') {
                $detallesIds = explode(',', $solicitud['detalles_liquidacion_ids']);
                if (!empty($detallesIds)) {
                    $placeholders = str_repeat('?,', count($detallesIds) - 1) . '?';
                    $sqlDetalle = "UPDATE compras.detalle_liquidaciones 
                SET comprobante_tesoreria = ?, 
                    fecha_transferencia = ?,
            ...
            return $this->res->ok('Comprobante actualizado correctamente');

        } catch (Exception $e) {
            if ($this->connect->inTransaction()) $this->connect->rollBack();
            error_log("Error en editarComprobanteTransferencia: " . $e->getMessage());
            return $this->res->fail('Error al editar comprobante', $e);
        }
    }
    // ============================================================================
    // HELPERS GENERALES
