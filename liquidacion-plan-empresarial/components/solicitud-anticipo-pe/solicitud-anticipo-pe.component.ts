...
export class SolicitudAnticipoPEComponent implements OnChanges {
  ...
  /** ¿Existe solicitud previa en curso?
 *   SOLO si hay seguimiento significativo Y el estado sea "pendiente / en proceso / en revisión"
 *   O si hay seguimiento pero aún NO hay fecha de autorización.
 */
  yaSolicitada(a: AnticipoPendientePE): boolean {
    if (!this.tieneSeguimiento(a)) return false; // <- clave: si no hay datos, NO está en curso

    const nombre = (a.ultimoSeguimiento?.nombreEstado || '').toLowerCase().trim();
    const enCursoPorNombre = ['pendiente', 'en proceso', 'en revisión', 'en revision'].includes(nombre);

    const sinAutorizacion = !a.ultimoSeguimiento?.fechaAutorizacion;

    // Si hay seguimiento y está marcado como pendiente/en proceso/en revisión, o aún no autorizado => en curso
    return enCursoPorNombre || sinAutorizacion;
  }
  /** ¿El último seguimiento trae algún dato relevante? */
  tieneSeguimiento(a: AnticipoPendientePE): boolean {
    const s = a.ultimoSeguimiento;
    if (!s) return false;

    const anyText = (v: any) => typeof v === 'string' && v.trim().length > 0;

    return Boolean(
      anyText(s.nombreEstado) ||
      anyText(s.descripcionEstado) ||
      anyText(s.comentarioSolicitante) ||
      anyText(s.comentarioAutorizador) ||
      s.fechaSeguimiento ||          // permite Date/ISO string no vacío
      s.fechaAutorizacion ||         // idem
      Number.isFinite(s.idEstado)    // id_estado con número
    );
  }
