...
export class ModalCrearSolicitudComponent implements OnInit {
    ...
    public readonly service = inject(LiquidacionesModuloTesoreriaService); // ← PUBLIC para usar en template

    @Input({ required: true }) factura!: FacturaConSolicitud; // ← Aseguramos que siempre llegue una factura
    @Input() modoEdicion = false;
    @Output() cerrar = new EventEmitter<void>();
    @Output() confirmado = new EventEmitter<void>();

    // Exponemos directamente lo que el template necesita
    ...
    // Bancos directamente desde el servicio (síncrono)
    ...
    async submit(): Promise<void> {
      ...
      `,
        ...
        const accion$ = this.esEdicion()
            ? this.service.editarSolicitudTransferencia(payload as any)
            : this.service.crearSolicitudTransferencia(payload as any);
