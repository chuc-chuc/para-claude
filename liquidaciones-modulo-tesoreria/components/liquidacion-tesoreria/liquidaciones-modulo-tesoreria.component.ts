...
import { LucideAngularModule, Eye, AlertCircle, Send, RefreshCw, FileText, CheckCircle, X, Check, PencilLine } from 'lucide-angular';
...
export class LiquidacionesModuloTesoreriaComponent implements OnInit, OnDestroy {
    ...
    // MODALES
    // ============================================================================
    cerrarModalDetalle(): void { this.mostrarModalDetalle.set(false); this.facturaSeleccionada.set(null); }
    cerrarModalCrear(): void { this.mostrarModalCrear.set(false); this.facturaSeleccionada.set(null); }
    cerrarModalComprobante(): void { this.mostrarModalComprobante.set(false); this.facturaSeleccionada.set(null); }

    onSolicitudCreada(): void { this.cerrarModalCrear(); this.refrescarDatos(); }
    onComprobanteRegistrado(): void { this.cerrarModalComprobante(); this.refrescarDatos(); }
    ...
    get esModoEdicionSolicitud(): boolean {
        return this.facturaSeleccionada()?.solicitud !== null;
    }

    get esModoEdicionComprobante(): boolean {
        return this.facturaSeleccionada()?.solicitud?.estado === 'completado';
    }

    mostrarBotonCrearSolicitud(f: FacturaConSolicitud) { return !f.solicitud && f.monto_pendiente_pago > 0; }
    mostrarBotonEditarSolicitud(f: FacturaConSolicitud) { return f.solicitud?.estado === 'rechazado' || f.solicitud?.estado === 'pendiente_aprobacion'; }
    mostrarBotonRegistrarComprobante(f: FacturaConSolicitud) { return f.solicitud?.estado === 'aprobado'; }
    mostrarBotonEditarComprobante(f: FacturaConSolicitud) { return f.solicitud?.estado === 'completado'; }
    mostrarBotonCancelar(f: FacturaConSolicitud) { return f.solicitud?.estado === 'pendiente_aprobacion'; }
}