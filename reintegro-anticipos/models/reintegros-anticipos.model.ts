 ...
 */
export type TipoOrden = 'plan-empresarial' | 'presupuesto';
...
export const TIPOS_ORDEN: OpcionSelect[] = [
    { valor: 'plan-empresarial', etiqueta: 'Plan Empresarial', descripcion: 'Ordenes por Plan Empresarial' }, // CAMBIO AQUÍ
    { valor: 'presupuesto', etiqueta: 'Por Presupuestos', descripcion: 'Orden por Presupuesto' },
];
...
// ============================================
// AGREGAR AL FINAL DE TU MODELO EXISTENTE
// ============================================

/**
 * Orden elegible para reintegro
 */
export interface OrdenPorTipo {
    numero_orden: number;
    total_anticipos: number;
    total_liquidado: number;
    total_reintegrado: number;
    saldo_pendiente: number;
    total_anticipos_formatted: string;
    saldo_pendiente_formatted: string;
}

/**
 * Respuesta de validación de orden elegible
 */
export interface ValidacionOrdenPorTipo {
    es_elegible: boolean;
    saldo_pendiente: number;
    total_anticipos: number;
    total_liquidado: number;
...
/**
 * Filtros para órdenes elegibles
 */
export interface FiltrosOrdenesPorTipo {
    tipo_orden?: string;
    monto_minimo?: number;
    monto_maximo?: number;
}

export class UtilidadesOrdenesPorTipo {
    static formatearParaSelect(orden: OrdenPorTipo): string {
        return `Orden #${orden.numero_orden} - Saldo: ${orden.saldo_pendiente_formatted}`;
    }

    static obtenerDescripcionDetallada(orden: OrdenPorTipo): string {
        return `Orden #${orden.numero_orden} - Anticipos: ${orden.total_anticipos_formatted} - Saldo: ${orden.saldo_pendiente_formatted}`;
    }

    static tieneSaldoSuficiente(orden: OrdenPorTipo, montoSolicitado: number): boolean {
        return orden.saldo_pendiente >= montoSolicitado;
    }
}
